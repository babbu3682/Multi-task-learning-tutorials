...
def get_args_parser():
    ...
    parser.add_argument('--png-save-dir', default='', help='path where to save, empty for no saving')
...
def main(args):
    ...
    if args.from_pretrained:
        ...
        if args.from_pretrained.split('/')[-1] == '[UpTASK]ResNet50_ImageNet.pth':
            ...
            print("Loading... Pre-trained")      
            model_dict = model.state_dict() 
            print("Check Before weight = ", model_dict['encoder._conv_stem.weight'].std().item())
            ...
            if args.load_weight_type == 'full':
                ...
                model_dict.update(filtered_dict)             
                model.load_state_dict(model_dict)   
            print("Check After weight  = ", model.state_dict()['encoder._conv_stem.weight'].std().item())
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.training_stream == 'Upstream':
            ...
            elif args.model_name == 'Up_SMART_Net_V2':
                train_stats = train_Up_SMART_Net_V2(model, criterion, data_loader_train, optimizer, device, epoch, args.print_freq, args.batch_size)
                print("Averaged train_stats: ", train_stats)
                valid_stats = valid_Up_SMART_Net_V2(model, criterion, data_loader_valid, device, args.print_freq, args.batch_size, epoch, args.png_save_dir)
                print("Averaged valid_stats: ", valid_stats)            
